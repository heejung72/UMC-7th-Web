...
// Defining Types for the TV show and actor data
interface TVShow {
  name: string;
...
const TVDetail = () => {
  const { tvId } = useParams<{ tvId: string }>(); // Type for tvId

  // Custom hook for fetching TV show and actor details
  const { data: tv, isLoading, isError } = useCustomFetch<TVShowResponse>(`/tv/${tvId}?language=ko-KR`);
  const { data: actors } = useCustomFetch<ActorResponse>(`/tv/${tvId}/credits?language=ko-KR`);
