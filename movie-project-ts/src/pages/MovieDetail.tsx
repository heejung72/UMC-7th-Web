...
// Defining Types for the Movie and Actor data
interface Movie {
  title: string;
...
const MovieDetail = () => {
  const { movieId } = useParams<{ movieId: string }>(); // Type for movieId

  // Custom hook for fetching movie and actor details
  const { data: movies, isLoading, isError } = useCustomFetch<MovieDetailResponse>(`/movie/${movieId}?language=ko-KR`);
  const { data: actors } = useCustomFetch<ActorResponse>(`/movie/${movieId}/credits?language=ko-KR`);
